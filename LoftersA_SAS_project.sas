...
DATE: Friday, January 09, 2015     TIME: 3:28:37 PM
...
/* -------------------------------------------------------------------
   ...
   Generated on: Friday, January 09, 2015 at 3:28:17 PM
...
LIBNAME EC100017 "P:\QAC\qac200\students\alofters\Assignments";
...
PROC SQL;
   ...
   SELECT t1.ADAPPT42, 
      ...
      FROM EC100017.meps_fullyr_2012_recodedsubset t1;
QUIT;
...
/* -------------------------------------------------------------------
   ...
   Generated on: Friday, January 09, 2015 at 3:28:18 PM
   By task: List Data
...
/* -------------------------------------------------------------------
   ...
   Generated on: Friday, January 09, 2015 at 3:28:19 PM
...
/* -------------------------------------------------------------------
   ...
   Generated on: Friday, January 09, 2015 at 3:28:20 PM
...
/* -------------------------------------------------------------------
   ...
   Generated on: Friday, January 09, 2015 at 3:28:21 PM
...
/*   START OF NODE: Categorize Aggregate Score Sources of Care Info   */
%LET _CLIENTTASKLABEL='Categorize Aggregate Score Sources of Care Info';
%LET _CLIENTPROJECTPATH='P:\QAC\qac200\students\alofters\Assignments\LoftersA_SAS_project_011215.egp';
%LET _CLIENTPROJECTNAME='LoftersA_SAS_project_011215.egp';

GOPTIONS ACCESSIBLE;
%_eg_conditional_dropds(WORK.QUERY_FOR_MEPS_FULLYR_0004);

PROC SQL;
   CREATE TABLE WORK.QUERY_FOR_MEPS_FULLYR_0004 AS 
   SELECT t1.sum_additionalcare, 
          ...
          /* sum_additionalcare_categorical */
            (CASE  
               WHEN t1.sum_additionalcare = 0 and t1.sum_additionalcare <=.9
               THEN 0
            WHEN t1.sum_additionalcare >=1 and t1.sum_additionalcare <=1.9
               THEN 1
            WHEN t1.sum_additionalcare >=2  and t1.sum_additionalcare <=2.9
               THEN 2
               ELSE 3
            END) LABEL="Aggregate Scores of Sources of Care Info" AS sum_additionalcare_categorical
...
/* -------------------------------------------------------------------
   ...
   Input Data: Local:WORK.QUERY_FOR_MEPS_FULLYR_0004
   Server:  Local
   ------------------------------------------------------------------- */

%_eg_conditional_dropds(WORK.SORT);
/* -------------------------------------------------------------------
   Sort data set Local:WORK.QUERY_FOR_MEPS_FULLYR_0004
   ------------------------------------------------------------------- */

PROC SQL;
	CREATE VIEW WORK.SORT AS
		SELECT T.sum_additionalcare, T.sum_additionalcare_categorical
	FROM WORK.QUERY_FOR_MEPS_FULLYR_0004(FIRSTOBS=1 ) as T
;
QUIT;
...
PROC FREQ DATA=WORK.SORT
	ORDER=INTERNAL
;
	TABLES sum_additionalcare /  SCORES=TABLE;
	TABLES sum_additionalcare_categorical /  SCORES=TABLE;
RUN;
/* -------------------------------------------------------------------
   End of task code.
   ------------------------------------------------------------------- */
...
PROC SQL;
	CREATE VIEW WORK.SORTTempTableSorted AS
		SELECT T.sum_additionalcare_categorical
	FROM WORK.QUERY_FOR_MEPS_FULLYR_0004 as T
;
QUIT;
TITLE;
TITLE1 "Distribution analysis of: sum_additionalcare_categorical";
FOOTNOTE;
FOOTNOTE1 "Generated by the SAS System (&_SASSERVERNAME, &SYSSCPL) on %TRIM(%QSYSFUNC(DATE(), NLDATE20.)) at %TRIM(%SYSFUNC(TIME(), TIMEAMPM12.))";
	ODS EXCLUDE EXTREMEOBS MODES MOMENTS;
...
PROC UNIVARIATE DATA = WORK.SORTTempTableSorted
		...
		MU0=0
;
	VAR sum_additionalcare_categorical;
	HISTOGRAM   sum_additionalcare_categorical / NORMAL	( 	W=1 	L=1 	COLOR=YELLOW  MU=EST SIGMA=EST)
